"""
consumir_v10.py — CONSUMO independiente de los modelos de la v10.
...
LO QUE CAMBIA RESPECTO DE consumir_v9.py: LA CUARTA CLASE
----------------------------------------------------------
Los modelos de la v10 tienen 4 salidas. Las tres primeras son el eje ordinal de saturación de
IA; la cuarta, `3_no_diapositiva`, es la COMPUERTA: cuando gana, la respuesta correcta no es
un nivel sino la ausencia de veredicto.

Este script lo refleja en el reporte: si gana la compuerta imprime "NO ES UNA DIAPOSITIVA" y
NO informa nivel de huella de IA. Mostrar "nivel 2 de saturación" para una foto de un
escritorio sería reproducir en la consola el fallo que la cuarta clase vino a cerrar.

  Un detalle que vale la pena probar a mano con `--ruta`: pasarle una foto cualquiera que no
  sea una diapositiva y comparar contra lo que devuelve el modelo de la v9 sobre la misma
  imagen. El de la v9 no puede abstenerse y contesta con confianza alta; el de la v10 rechaza.
  Es la demostración más corta de para qué sirve la versión.

LOS TRES MODELOS, UNA INTERFAZ
------------------------------
    a  ->  TensorFlow/v10/modelotf_v10_finetune.keras    (Keras, NHWC, ya devuelve softmax)
    b  ->  app/src/assets/modelos/modelo_b_v10.onnx      (ONNX,  NCHW, devuelve logits)
    c  ->  app/src/assets/modelos/modelo_c_v10.onnx      (ONNX,  NCHW, devuelve logits)
...
el .onnx es autónomo (no necesita ni PyTorch ni el código de la clase para ejecutarse), que es
justamente el argumento por el que se exporta a ONNX.
...
Uso:
    ...
    PyTorch/.venv/Scripts/python modelos/v10/consumir_v10.py --modelo b --ruta foto.jpg

    # los tres a la vez sobre la misma carpeta, comparando
...
CLASES = ["0_sin_ia", "1_rastro_ia", "2_saturada_ia", "3_no_diapositiva"]
TITULOS = {
    "0_sin_ia": "Sin rastro de IA",
    "1_rastro_ia": "Rastro de IA",
    "2_saturada_ia": "Saturada de IA",
    "3_no_diapositiva": "NO es una diapositiva",
}
...
def cargar(clave: str):
    """Devuelve (predecir(array) -> probabilidades, cfg)."""
    cfg = MODELOS[clave]
...
# --- 3. Reporte ---
def imprimir(nombre_archivo: str, probs: np.ndarray, ms: float) -> int:
    ...
    "0.51 / 0.49 / 0.00 / 0.00" y "0.99 / 0.01 / 0.00 / 0.00" tienen la misma clase ganadora
    y son dos respuestas completamente distintas.

    La novedad de la v10 es la línea final: si gana la compuerta NO se reporta un nivel de
    huella de IA, porque no lo hay. Una raya separa visualmente el eje ordinal de la
    compuerta, para que no se lean como cuatro escalones de lo mismo.
    """
    n = min(len(probs), len(CLASES))
    ganador = int(np.argmax(probs[:n]))
    print(f"\n  {nombre_archivo}")
    for i in range(n):
        if i == N_ORDINALES:
            print("    " + "·" * 56 + "   (fuera del eje ordinal)")
        ...
        print(f"    {TITULOS[CLASES[i]]:<22} {probs[i]:6.2%}  {barra:<30}{marca}")

    conf = probs[ganador]
    if ganador >= N_ORDINALES:
        print(f"    -> NO ES UNA DIAPOSITIVA   (confianza {conf:.1%} · {ms:.0f} ms)")
        print("       No se informa nivel de huella de IA: no corresponde. Reencuadrá sobre")
        print("       la pantalla y volvé a disparar.")
        return ganador

    print(f"    -> {TITULOS[CLASES[ganador]]}   (confianza {conf:.1%} · {ms:.0f} ms)")
    # El umbral sigue al azar en vez de estar clavado: con 4 clases el azar es 25%, y el 0.60
    # que usaba la v9 (calibrado sobre 3 clases) habría convertido el aviso en ruido.
    azar = 1.0 / max(2, n)
    if conf < azar * 1.8:
        print(f"       AVISO: confianza baja. Sobre {n} clases el azar es {azar:.0%}: el "
              f"modelo está dudando.")
    return ganador
...
def modo_carpeta(claves: list[str], ruta: Path) -> None:
    ...
    for clave in claves:
        ...
        rechazadas = sum(1 for g in ganadores if g >= N_ORDINALES)
        if rechazadas:
            print(f"\n  {rechazadas}/{len(archivos)} imágenes rechazadas por la compuerta "
                  f"(no son diapositivas).")
    ...
    if len(resultados) > 1:
        ...
        for i, archivo in enumerate(archivos):
            ...
            if len(set(votos)) > 1:
                desacuerdos += 1
                detalle = "  ".join(f"{k}={TITULOS[CLASES[v]]}"
                                    for k, v in zip(claves_ok, votos))
        ...
        if desacuerdos:
            ...
            print("  OJO con un desacuerdo particular de la v10: si un modelo rechaza y otro")
            print("  informa nivel, no es una discrepancia de grado sino de si la imagen es")
            print("  siquiera analizable. Esa es la que hay que mirar primero.")
    ...
    try:
        while True:
            ...
            cv2.putText(frame, ultimo, (10, 65), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                        (255, 200, 0), 2)
            cv2.imshow("Is it AI? — consumo v10", frame)
            ...
            if tecla == ord(" "):
                ...
                ultimo = ("NO es una diapositiva" if g >= N_ORDINALES
                          else f"{TITULOS[CLASES[g]]} ({probs[g]:.0%})")
    finally:
        captura.release()
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Consumo independiente de los modelos v10 (no entrena nada).")
